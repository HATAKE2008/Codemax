...
object GitFetchTask {
       ...
       Git.open(project.getRootFile()).fetch().setTransportConfigCallback(sshTransportConfigCallback).setProgressMonitor(progress).call()
       ...
       else { Toast.makeText(context, context.getString(R.string.fetch_completed), Toast.LENGTH_SHORT).show() }     
