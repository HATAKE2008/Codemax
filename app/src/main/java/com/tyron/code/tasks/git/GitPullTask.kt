...
object GitPullTask {
  ...
  fun pull(project: Project, context: Context) {
      ...
      executeAsyncProvideError(
        {
          ...
          pullCommand.setProgressMonitor(progress)

          if (remoteName != null) {
            pullCommand.setRemote(remoteName)
          } else {
            pullCommand.setRemote(remoteUrl)
          }

          if (isSsh) {
            pullCommand.setTransportConfigCallback(GitCredentials.sshTransportConfigCallback)
          } else {
